...
function validateName() {
  ...
  if (value.length < 3) {
    setError(errorEl, "Name must be at least 3 characters.");
    setInvalid(input, true);
    return false;
  }
  setError(errorEl, "");
...
function validateYob() {
  ...
  if (raw.length === 0) {
    setError(errorEl, "Year of birth is required.");
    setInvalid(input, true);
    return false;
  }
  if (!/^\d+$/.test(raw)) {
    setError(errorEl, "Enter an integer year (numbers only).");
    setInvalid(input, true);
    return false;
  }
  const year = parseInt(raw, 10);
  if (year <= 1900) {
    setError(errorEl, "Year must be greater than 1900.");
    setInvalid(input, true);
    return false;
  }
  if (year >= currentYear) {
    setError(errorEl, `Year must be less than ${currentYear}.`);
    setInvalid(input, true);
    return false;
  }
  setError(errorEl, "");
...
function validateZip() {
  ...
  if (raw.length === 0) {
    setError(errorEl, "Zipcode is required.");
    setInvalid(zip, true);
    return false;
  }
  if (!/^\d{5}$/.test(raw)) {
    setError(errorEl, "Zipcode must be 5 digits.");
    setInvalid(zip, true);
    return false;
  }
  setError(errorEl, "");
...
function validatePizza() {
  ...
  if (!checked) {
    setError(errorEl, "Please select one option.");
    return false;
  }
  setError(errorEl, "");
  return true;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", e => {
    ...
    const validYob = validateYob();
    const validZip = validateZip();
    const validPwd = validatePassword();
    const validPizza = validatePizza();
    const ok = validName && validYob && validZip && validPwd && validPizza;
    if (ok) {
